...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        $payload = ['name'=>trim($_POST['name']??''),'anime_name'=>trim($_POST['anime_name']??''),'bio'=>trim($_POST['bio']??''),'abilities'=>trim($_POST['abilities']??''),'image_url'=>trim($_POST['image_url']??'')];
...
<form method="post" class="editor-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?= (int)($editing['id'] ?? 0) ?>"><section class="admin-card admin-form"><div class="form-row"><label>Name<input name="name" value="<?= $value('name') ?>" required></label><label>Original series<input name="anime_name" value="<?= $value('anime_name') ?>" required></label></div><label>Biography<textarea name="bio" rows="8" required><?= $value('bio') ?></textarea></label><label>Abilities<textarea name="abilities" rows="5" required><?= $value('abilities') ?></textarea></label></section><aside class="admin-card admin-form sticky-editor"><label>Image URL<input name="image_url" value="<?= $value('image_url', '/assets/images/character-blue.svg') ?>" required></label><div class="image-preview portrait"><img id="image-preview" src="<?= $value('image_url', '/assets/images/character-blue.svg') ?>" alt="Preview"></div><button class="button primary full" type="submit"><?= $editing ? 'Save changes' : 'Create character' ?> →</button></aside></form>
<?php else: ?>
