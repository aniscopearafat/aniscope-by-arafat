...
document.querySelectorAll('[data-image-setting]').forEach(setting => {
  const textInput = setting.querySelector('[data-setting-image-input]');
  const fileInput = setting.querySelector('[data-setting-file]');
  const preview = setting.querySelector('[data-setting-preview]');
  textInput?.addEventListener('input', () => {
    if (!preview) return;
    const htmlMatch = textInput.value.match(/<img\b[^>]*\bsrc\s*=\s*["']([^"']+)["']/i);
    preview.src = htmlMatch ? htmlMatch[1] : textInput.value;
  });
...
document.querySelectorAll('.color-field').forEach(field => {
  ...
  text?.addEventListener('input', () => {
    if (/^#[0-9a-fA-F]{6}$/.test(text.value) && picker) picker.value = text.value;
  });
});
